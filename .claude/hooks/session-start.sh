...
if ! command -v deno >/dev/null 2>&1; then
  case "$(uname -m)" in
    ...
    *) echo "session-start: unsupported arch $(uname -m), skipping Deno install" >&2; exit 0 ;;
  esac
  ...
  curl -fsSL --retry 3 --retry-delay 2 -o "$tmp/deno.zip" "$url"
  unzip -oq "$tmp/deno.zip" -d "$tmp"
  install -m 0755 "$tmp/deno" "$INSTALL_DIR/deno"
fi
